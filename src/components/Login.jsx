...
export default function Login() {
  ...
  async function login(info) {
    const response = await axios.post("https://ecom-project-backend-gwbx.onrender.com/auth/login", info)
    ...
    <>
     ...
     <input type="email" value={email} placeholder="Email" onChange={(e) => setEmail(e.target.value)} />
      ...
      {loginMutation.isError && (
        <div>{loginMutation.error?.response?.data?.message || loginMutation.error.message}</div>
      )}
    </div>
